...
print("=" * 80)
print("Swap Attack Data Analyzer for Logit Swap Attack")
...
try:
    with pd.read_csv(INPUT_FILENAME, chunksize=CHUNK_SIZE) as reader:
        for chunk in tqdm(reader, desc="Processing chunks"):
            ...
            # Calculate the swap attack-induced difference (reconstructed calc vs original real)
            paired['abs_swap_difference'] = (
            ...
            reconstruction_groups = paired.groupby(['input', 'reconstruction_idx'])['abs_swap_difference'].mean()
            ...
            if not valid_layers.empty:
                layer_groups = valid_layers.groupby('block_number')['abs_swap_difference']
            ...
            if not valid_layers.empty:
                component_groups = valid_layers.groupby('component_type')['abs_swap_difference']
...
for (input_id, recon_idx), values in reconstruction_accumulator.items():
    reconstruction_data.append({
        ...
        'mean_swap_difference': np.mean(values)
    })
...
reconstruction_df.to_csv(os.path.join(OUTPUT_DIR, "aggregated_swap_reconstructions.csv"), index=False)
layer_stats_df.to_csv(os.path.join(OUTPUT_DIR, "aggregated_swap_layer_stats.csv"), index=False)
component_stats_df.to_csv(os.path.join(OUTPUT_DIR, "aggregated_swap_component_stats.csv"), index=False)
...
print("\n" + "=" * 80)
print("🔍 LOGIT SWAP ATTACK ANALYSIS")
print("=" * 80)
...
print(f"\n📈 Swap Attack Effectiveness Summary:")
stats = {
...
# Extreme Cases
print(f"\n🔥 Highest Swap Attack Impact (Top 5):")
top_5 = reconstruction_df.nlargest(5, 'mean_swap_difference')
for i, (_, row) in enumerate(top_5.iterrows(), 1):
    print(f"   {i}. {row['mean_swap_difference']:.6f} → Input {row['input']}, Recon #{row['reconstruction_idx']}")

print(f"\n❄️  Lowest Swap Attack Impact (Top 5):")
bottom_5 = reconstruction_df.nsmallest(5, 'mean_swap_difference')
...
print(f"   • {high_impact_pct:.1f}% of reconstructions show high swap impact (>0.01)")
print(f"   • {low_impact_pct:.1f}% of reconstructions show low swap impact (<0.001)")
print(f"   • Average swap-induced difference: {sorted_data.mean():.6f}")
...
# Per-input analysis
print(f"\n📊 Per-Input Swap Attack Success:")
input_stats = reconstruction_df.groupby('input')['mean_swap_difference'].agg(['mean', 'std', 'min', 'max', 'count'])
...
print("\n" + "=" * 80)
print("✅ SWAP ATTACK DATA ANALYSIS COMPLETE!")
print("=" * 80)
print("\nGenerated files:")
print("  • aggregated_swap_reconstructions.csv - Reconstruction statistics")
print("  • aggregated_swap_layer_stats.csv - Layer-level vulnerability")
print("  • aggregated_swap_component_stats.csv - Component-level vulnerability")
print("\nRun swap_attack_visualizer.py to generate plots from these aggregated statistics.")
print("=" * 80)
