...
print("=" * 80)
print("Data Analyzer for LLM-TS3 Analysis")
...
if not os.path.exists(INPUT_FILENAME):
    ...
    print("Please make sure formula_analysis.csv is in the current directory.")
    sys.exit(1)
...
test_case_accumulator = defaultdict(list)  # (input, token_pos) -> list of differences
...
try:
    with pd.read_csv(INPUT_FILENAME, chunksize=CHUNK_SIZE) as reader:
        for chunk in tqdm(reader, desc="Processing chunks"):
            ...
            # Filter to keep only original and other
            filtered = chunk[
            ...
            # Pivot to pair up original and other
            try:
                paired = filtered.pivot_table(
                    index=['input', 'token_pos', 'round', 'layer_name'],
                    columns='run_type',
            ...
            if 'error_calc_value_other' not in paired.columns or 'error_real_value_original' not in paired.columns:
                skipped_chunks += 1
                continue
            
            # Calculate the cross-model difference
            paired['abs_calculation_difference'] = (
                paired['error_calc_value_other'] - paired['error_real_value_original']
            ).abs()
            ...
            # Accumulate test case statistics (input, token_pos)
            test_case_groups = paired.groupby(['input', 'token_pos'])['abs_calculation_difference'].mean()
            for key, value in test_case_groups.items():
                test_case_accumulator[key].append(value)
            
            # Accumulate layer statistics (for valid blocks only)
            valid_layers = paired[paired['block_number'] >= 0]
            if not valid_layers.empty:
                layer_groups = valid_layers.groupby('block_number')['abs_calculation_difference']
            ...
            if not valid_layers.empty:
                component_groups = valid_layers.groupby('component_type')['abs_calculation_difference']
...
test_case_data = []
for (input_id, token_pos), values in test_case_accumulator.items():
    test_case_data.append({
        'input': input_id,
        'token_pos': token_pos,
        'mean_cross_model_difference': np.mean(values)
    })
...
print(f"  Test cases: {len(test_case_df):,}")
print(f"  Layers: {len(layer_stats_df)}")
print(f"  Components: {len(component_stats_df)}")

# Save aggregated data
test_case_df.to_csv(os.path.join(OUTPUT_DIR, "aggregated_test_cases.csv"), index=False)
layer_stats_df.to_csv(os.path.join(OUTPUT_DIR, "aggregated_layer_stats.csv"), index=False)
component_stats_df.to_csv(os.path.join(OUTPUT_DIR, "aggregated_component_stats.csv"), index=False)
...
print(f"\n📊 Dataset Overview:")
print(f"   Test cases analyzed: {len(test_case_df):,}")
print(f"   Total rows processed: {total_rows_processed:,}")
print(f"   Unique inputs: {test_case_df['input'].nunique()}")
print(f"   Unique token positions: {test_case_df['token_pos'].nunique()}")
...
top_5 = test_case_df.nlargest(5, 'mean_cross_model_difference')
for i, (_, row) in enumerate(top_5.iterrows(), 1):
    print(f"   {i}. {row['mean_cross_model_difference']:.6f} → Input {row['input']}, Token Pos {row['token_pos']}")

print(f"\n❄️  Lowest Differences (Top 5):")
bottom_5 = test_case_df.nsmallest(5, 'mean_cross_model_difference')
...
if len(layer_stats_df) > 0:
    most_different_layer = layer_stats_df.loc[layer_stats_df['mean'].idxmax()]
    least_different_layer = layer_stats_df.loc[layer_stats_df['mean'].idxmin()]
    
    print(f"   • Most different layer: {int(most_different_layer['layer'])} (avg: {most_different_layer['mean']:.6f})")
    print(f"   • Least different layer: {int(least_different_layer['layer'])} (avg: {least_different_layer['mean']:.6f})")

print("\n" + "=" * 80)
print("✅ DATA ANALYSIS COMPLETE!")
print("=" * 80)
print("\nGenerated files:")
print("  • aggregated_test_cases.csv - Test case statistics")
print("  • aggregated_layer_stats.csv - Layer-level statistics")
print("  • aggregated_component_stats.csv - Component-level statistics")
print("\nRun visualizer.py to generate plots from these aggregated statistics.")
print("=" * 80)
